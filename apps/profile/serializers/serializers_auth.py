...
#create user dulu tanpa sekaligus create profile
...
#create user langsung create profile juga
...
class UserRegisterSerializer(serializers.ModelSerializer):
    ...
    alamat = serializers.CharField(write_only=True)
    kota = serializers.CharField(write_only=True)
    nik_group = serializers.CharField(write_only=True)
    nik_lokal = serializers.CharField(write_only=True)
    ...
    class Meta:
        model = User
        fields = ['username', 'email', 'password', 'password_confirm', 'nama_lengkap', 'phone_number', 'organisasi','alamat','kota','nik_group','nik_lokal']
        extra_kwargs = {'password': {'write_only': True}}
    ...
    def create(self, validated_data):
        ...
        profile_data = {
            ...
            'nik_group': validated_data['nik_group'],
            'nik_lokal': validated_data['nik_lokal'],
            'role': "User"
        }
